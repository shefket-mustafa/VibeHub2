...
postRoutes.delete("/delete/:id", authMiddleware, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: "Not authenticated!" });
        }
...
postRoutes.patch("/:id/like", authMiddleware, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: "Not authenticated!" });
        }
...
postRoutes.get("/user/:id", async (req, res) => {
    try {
...
postRoutes.post("/:id/comments", authMiddleware, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: "Not Authenticated!" });
        const { content } = req.body;
