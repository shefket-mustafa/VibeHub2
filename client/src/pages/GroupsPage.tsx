...
export default function GroupsPage() {
  ...
  const { user } = useUser();
  const { groupInfoModalOpen, selectedGroupHandler, selectedGroup } =
    useGroups();
  const { t } = useTranslation();

  const {
    data: discoverGroups = [],
    error: errorDiscoverGroups,
    isLoading: isLoadingDiscoverGroups,
  } = useGetAllGroupsQuery();
  const {
    data: yourGroups = [],
    error: errorMyGroups,
      ...
      <div className="w-full min-h-screen  flex justify-between z-10">
        <div className="w-full flex-1 max-w-5xl mx-auto space-y-6 py-20 px-5">
          ...
          <ul className="space-y-4">
            {activeTab === "your" ? (
              yourGroups.length === 0 ? (
                <p className="text-sm muted">{t("groups.main.notJoined")}</p>
              ) : (
                yourGroups.map((g) => (
                  <li
                    key={g._id}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-neutral-400">
                          {g.members?.length}{" "}
                          {g.members?.length === 1
                            ? t("groups.main.member")
                            : t("groups.main.members")}
                        </p>
                      </div>
                    </div>
              ...
              <p className="text-sm muted">{t("groups.main.noDiscover")}</p>
            ) : (
              discoverGroups?.map((g) => (
                ...
                >
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={() => selectedGroupHandler(g)}
                      className="text-sm text-black bg-orange-400 hover:bg-orange-500 px-3 py-1 cursor-pointer rounded-lg transition"
                    >
                      {t("groups.main.info")}
                    </button>
